...
final class AgentProviderRegistry {
    ...
    ///
    /// Note: `QoderProvider`, `QoderWorkProvider`, `CursorProvider`, and
    /// `CodexProvider` are registered here so the Settings UI can list them
    /// as available choices, but they are intentionally absent from the
    /// default `enabledProviders` set (`["claude"]`). As long as `enabled()`
    /// excludes them, scanners / watchers / hook installers never touch
    /// `~/.qoder/`, `~/.qoderwork/`, `~/.cursor/`, or `~/.codex/`.
